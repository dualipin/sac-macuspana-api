...
class SolicitudViewSet(viewsets.ModelViewSet):
    ...
    def verificar_documentacion(self, request, pk=None):
        ...
        requisitos_subidos = set(
            documentos_subidos.values_list("requisito_id", flat=True)
        )
            ...
            requisitos_info.append(
                {
                    ...
                    "documento_subido": req.id in requisitos_subidos,
                }
            )
...
class DashboardView(APIView):
    ...
    def get(self, request):
        ...
        if user.rol == Roles.CIUDADANO and hasattr(user, "ciudadano"):
            ...
            return Response({"detail": "Rol no reconocido"}, status=status.HTTP_403_FORBIDDEN)
        ...
        for item in conteo:
            data["resumen_estatus"].append({
                ...
                "estatus_label": dict(EstatusSolicitud.choices).get(item["estatus"], item["estatus"]),
                "total": item["total"]
            })
...
class AdminDashboardViewSet(viewsets.ViewSet):
    ...
    def stats(self, request):
        ...
        requests_today = Solicitud.objects.filter(updated_at__date=today).count() # Using updated_at as proxy if created_at not avail or for activity
        
        active_departments = Dependencia.objects.count()
        
        # Avg response time: Start (created_at) to End (updated_at when status is final)
        completed_qs = Solicitud.objects.filter(estatus__in=[EstatusSolicitud.APROBADO, EstatusSolicitud.RECHAZADO])
        ...
        status_distribution = Solicitud.objects.values('estatus').annotate(count=Count('estatus'))
    ...
    def trends(self, request):
        ...
        elif period == '90days': days = 90
        
        start_date = timezone.now() - timedelta(days=days)
        
        qs = Solicitud.objects.filter(created_at__gte=start_date)
        
        by_date = qs.annotate(date=TruncDate('created_at')).values('date').annotate(
            created=Count('id'),
    ...
    def requests(self, request):
        qs = Solicitud.objects.all().select_related('ciudadano', 'tramite_tipo', 'programa_social')
        ...
        serializer = SolicitudListSerializer(qs, many=True, context={'request': request})
        return Response(serializer.data)
